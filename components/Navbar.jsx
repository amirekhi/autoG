...
const Navbar = () => {
  ...
  return (
    <header className=' relative max-w-full lg:max-w-[95%] 2xl:max-w-[80%] mx-auto z-50 h-[120px]  flex text-black justify-around items-center   max-2xl:justify-between bg-white' >
      <Link href={'/'} className={`${NavSession != '' && ('max-md:hidden')}`} ><Image src={logo} height={80} alt='menu' className='object-contain  ml-8 -translate-y-6  hover:-translate-y-8 transition  duration-300 ease-in-outs '  /></Link> 
      {NavSession != '' ? (<div className='flex justify-center items-center gap-3'><h3 className='text-2xl font-semibold max-md:pl-6'>{ shortenString(NavSession.userId , 8)}</h3><button className='w-[100px] h-[50px] rounded-full hover:scale-110 transition duration-200   bg-red-400 text-white max-md:w-[80px] ' onClick={ async () => { await logout()
       window.location.reload() }} >LogOut</button> </div>) : (<div className='flex gap-4 w-[33%] py-4 justify-center items-center h-full max-sm:flex-col '><button className='w-[100px] h-[50px] rounded-full hover:scale-110 transition duration-200   bg-gray-400 text-white max-md:w-[80px] ' onClick={()=> {redirect('/Sign-Up')}} >Sign Up</button> <button className='w-[100px] h-[50px] rounded-full   hover:scale-110 transition duration-200 bg-blue-500 text-white  max-md:w-[80px]' onClick={()=> {redirect('/login')}} >Log In</button> </div>)}
      ...
      <nav ref={menuRef} className="xl:hidden flex  justify-end w-[20%] items-center z-50 ">
          ...
          >
            <ul className="list-none flex relative z-50 justify-end items-start flex-col gap-4">
              {navLinks.map((link) => (
                <li key={link.path}>
                  <Link 
                    href={link.path} 
